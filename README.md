...
- **Трейсы** — HTTP-запрос к `order` и его gRPC-вызовы к `inventory` и `payment`
  складываются в один трейс в Jaeger.

Чего пока нет: **контекст трассировки не передаётся через Kafka**. `assembly` и
`notification` в Jaeger не видны — их работа не пришивается к трейсу заказа.
Чтобы это починить, нужно класть `traceparent` в заголовки сообщения на стороне
продюсера и доставать его в consumer middleware.
