// Package app собирает и запускает сервис inventory.
package app
...
// seeder умеет наполнить пустой каталог демо-данными.
type seeder interface {
	Seed(ctx context.Context) error
}

// App — приложение inventory со всеми его серверами.
type App struct {
...
func (a *App) initGRPCServer(ctx context.Context) error {
	...
	// Reflection нужен, чтобы grpcurl видел схему сервиса без .proto под рукой.
...
// initSeed наполняет каталог демо-деталями, если он пуст.
func (a *App) initSeed(ctx context.Context) error {
	repo, ok := a.diContainer.PartRepository(ctx).(seeder)
	if !ok {
		return nil
	}
...
func (a *App) runGRPCServer(ctx context.Context) error {
	logger.Info(ctx, fmt.Sprintf("🚀 gRPC InventoryService слушает на %s", config.AppConfig().GRPC.Address()))
